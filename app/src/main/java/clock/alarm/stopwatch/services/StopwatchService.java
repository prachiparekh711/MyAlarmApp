...
import static clock.alarm.stopwatch.activities.MainActivity.EXTRA_FRAGMENT;
import static clock.alarm.stopwatch.activities.MainActivity.FRAGMENT_STOPWATCH;
...
public class StopwatchService extends Service {
        ...
        runnable = new Runnable() {
            ...
            public void run() {
                if (isRunning) {
                    ...
                    String text = FormatUtils.formatMillis1(currentTime);
                    if (listener != null)
                    ...
                    listener.onTick(time, FormatUtils.formatMillis1(time));
                }
            }
        };
    ...
    private Notification getNotification(String time) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
            notificationManager.createNotificationChannel(new NotificationChannel(AlarmApp.NOTIFICATION_CHANNEL_STOPWATCH, getString(R.string.title_stopwatch), NotificationManager.IMPORTANCE_DEFAULT));

        return new NotificationCompat.Builder(this, AlarmApp.NOTIFICATION_CHANNEL_STOPWATCH)
